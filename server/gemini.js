...
function buildPrompt(input) {
  ...
  const lines = [
    ...
    ``,
    `САБАҚ МАҚСАТЫ — ҮШ ДЕҢГЕЙЛІ ДИФФЕРЕНЦИАЦИЯ (МІНДЕТТІ, РЕСМИ ҚМЖ ҮЛГІСІ):`,
    `- "lessonObjectives" өрісі міндетті түрде үш бөлек тұжырымнан тұрады: "all" (Барлық оқушылар үшін — ең төменгі, міндетті нәтиже), "most" (Көпшілік оқушылар үшін — стандартты деңгей, есептер шығару/қолдану), "some" (Кейбір оқушылар үшін — жоғары деңгей, талдау/жинақтау/өз бетінше шешім табу).`,
    `- Үш деңгей бір-бірінен нақты ажыратылсын (тек сөз жамау емес, шынымен күрделілігі өспелі болсын), Bloom таксономиясы деңгейлеріне сай (білу → қолдану → талдау).`,
    ``,
    ...
    `- "tasks" өрісі — тапсырмалар тізімі, саны еркін (әдетте 1-4). Екі үлгінің бірін таңда, сабақтың сипатына қарай: (а) ЖЕКЕ/ЖҰПТЫҚ жұмыс болса — сыныпқа ортақ, БІРІН СОҢ БІРІ орындалатын нөмірленген тапсырмалар («1-тапсырма», «2-тапсырма» т.б., "level" өрісінде осы нөмірді жаз); (ә) ТОПТЫҚ жұмыс болса — деңгейлік тапсырмалар (Базалық/Негізгі/Жетілдірілген), әрқайсысына бөлек критерий, дескриптор, балл.`,
    ...
    ``,
    `СУРЕТ/СЫЗБА ОРНЫ (imageHint):`,
    `- Сен сурет сала алмайсың. Егер тапсырманы түсіну үшін сурет, сызба, диаграмма немесе фигура шынымен қажет болса (мысалы: пайыздық фигураны бояу, геометриялық сурет, кесте-диаграмма, тәжірибе схемасы), сол тапсырманың "imageHint" өрісіне НЕ салу керектігін қысқа әрі нақты сипатта (мысалы: «Тіктөртбұрыш, 40%-ы боялған»). Мұғалім бұл сипаттама бойынша өзі сурет қосады.`,
    `- "imageHint"-ті тек шынымен қажет болғанда ғана қос, әр тапсырмаға міндетті түрде қоспа.`,
  ];
